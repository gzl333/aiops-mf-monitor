...
import ServerCluster from 'components/server/ServerCluster.vue'
...
const getServerQuery = async (monitor_unit_id: string) => {
  ...
  for (const query of serverQuery) {
    ...
    const MonitorServerQueryRes = await service.monitor.getMonitorServerQuery(config)
...
const getAllUnit = async () => {
  for (const organization of organizations.value) {
    ...
    for (let i = 0; i < numberRequest; i++) {
      ...
      const monitorUnitServerRes = await service.monitor.getMonitorUnitServer({
        query: {
          page: i + 1,
          page_size: 100,
          organization_id: organization.id
