...
import CephCluster from 'components/ceph/CephCluster.vue'
...
const getStorageQuery = async (monitor_unit_id: string) => {
  ...
  for (const query of storageQuery) {
    ...
    const MonitorCephQuery = await service.monitor.getMonitorCephQuery(config)
...
const getAllUnit = async () => {
  for (const organization of organizations.value) {
    ...
    for (let i = 0; i < numberRequest; i++) {
      ...
      const monitorUnitServerRes = await service.monitor.getMonitorUnitCeph({
        query: {
          page: i + 1,
          page_size: 100,
          organization_id: organization.id
